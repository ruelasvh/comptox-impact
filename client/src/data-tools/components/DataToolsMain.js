...
class DataToolsMain extends React.Component {
    ...
    render() {
      ...
      const pages = tablist.map((tab) => (
          <Tab.Pane unmountOnExit={true} mountOnEnter={true} key={tab.name} eventKey={tab.eventKey}>
              <br/>{tab.description}<br/><br/><Subtab tab={tab} self={this.props.analytics}/>
          </Tab.Pane>)
      )


        const dataDomainTabInstance = (
            <Tab.Container defaultActiveKey="comptoxdashboard" id="data-tools-master-tab-instance">
