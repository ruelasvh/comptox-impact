...
const DoughnutChart = ({ data }) => {
  ...
  const aggregatedBlacklist = blacklist.concat(blacklistTerms, blacklistFtpLogs)
  
  const isInBlacklist = (blacklist, domain) => {
    return !blacklist.every(item => {
      return !domain.toLowerCase().includes(item.toLowerCase());
    })
  }
